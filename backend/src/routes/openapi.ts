...
export function buildSpec(base: string) {
  ...
  return {
    ...
    paths: {
      ...
      '/requerimientos/{id}/historial': { get: { operationId: 'getHistorialRequerimiento', summary: 'Reprogramaciones (fecha anterior, nueva, motivo) y reprocesos (origen, causa, paso, horas) de la tarea', parameters: [path('id')], responses: { '200': { description: 'HistorialRequerimiento' } } } },
      '/requerimientos/{id}/reprocesos': { post: { operationId: 'registrarReproceso', summary: 'El entregable vuelve al equipo: registra causa de catálogo, reabre la tarea y el to-do en Basecamp', parameters: [path('id')], requestBody: { required: true, content: json({ type: 'object', properties: { origen: { type: 'string', enum: ['cliente', 'interno'] }, motivo: { type: 'string', enum: ['brief_incompleto', 'error_ejecucion', 'cambio_opinion_cliente', 'ajuste_marca_legal', 'direccion_arte', 'error_texto'] }, paso_retorno: { type: 'string', nullable: true }, reabrir_basecamp: { type: 'boolean', default: true } }, required: ['origen', 'motivo'] }) }, responses: { '201': { description: 'Reproceso' } } } },
      '/requerimientos/causas-pendientes': { get: { operationId: 'getCausasPendientes', summary: 'Reprogramaciones y reprocesos sin causa registrada (45 días)', responses: { '200': { description: 'Pendientes' } } } },

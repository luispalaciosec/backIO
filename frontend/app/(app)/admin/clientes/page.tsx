            ...
            <div className="flex gap-1">
              ...
              {c.basecamp_project_id && (
                <button type="button" className={`${c.basecamp_importado_at ? 'btn-ghost' : 'btn-primary'} text-xs`} title={c.basecamp_importado_at ? `Importado ${new Date(c.basecamp_importado_at).toLocaleString('es-EC')}. Volver a importar trae solo lo nuevo.` : 'Trae las listas y to-dos existentes del proyecto Basecamp (solo títulos, nunca comentarios)'} onClick={async () => {
                  if (!confirm(`Sincronizar "${c.nombre}" con Basecamp: trae to-dos nuevos y actualiza títulos, listas, grupos, responsables y eliminados. Solo entran títulos, fechas, asignados y estado; nunca descripciones ni comentarios. ¿Continuar?`)) return;
                  setError(null); setOk(null);
                  try { const r = await api<{ listas: number; proyectos_creados: number; requerimientos_creados: number; ya_enlazados: number; omitidos_completados_viejos: number; responsables_actualizados: number; titulos_actualizados: number; movidos: number; proyectos_renombrados: number; eliminados_en_basecamp: number }>(`/clientes/${c.id}/basecamp/importar`, { method: 'POST' }); setOk(`${c.nombre}: ${r.listas} listas · ${r.proyectos_creados} proyectos nuevos · ${r.requerimientos_creados} to-dos importados · ${r.ya_enlazados} ya enlazados · ${r.titulos_actualizados} títulos actualizados · ${r.movidos} movidos de lista · ${r.proyectos_renombrados} listas renombradas · ${r.responsables_actualizados} responsables actualizados · ${r.eliminados_en_basecamp} eliminados en Basecamp (cancelados aquí)`); await cargar(); }
                  catch (e) { setError(e instanceof ApiError ? e.message : 'Error'); }
                }}>{c.basecamp_importado_at ? '↻ Sincronizar Basecamp' : 'Importar Basecamp'}</button>
              )}

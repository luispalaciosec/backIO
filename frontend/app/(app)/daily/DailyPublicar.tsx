...
export function DailyPublicar({ mesa, mesaNombre, tieneBoard }: { mesa: string; mesaNombre: string; tieneBoard: boolean }) {
  ...
  async function redactar(tipo: 'apertura' | 'cierre') {
    ...
    try {
      const r = await api<{ texto: string; resumen: { hoy: number; vencen: number; bloqueos: number; cambios: number; completadas?: number } }>('/ia/daily', { method: 'POST', json: { mesa_id: mesa, tipo, notas: notasArr() } });
      setNarrativa(r.texto);
      setMsg({ tipo: 'info', texto: `Borrador de ${tipo} listo: ${tipo === 'cierre' ? `${r.resumen.completadas ?? 0} completadas hoy, ${r.resumen.hoy} quedaron abiertas` : `${r.resumen.hoy} para hoy`}, ${r.resumen.vencen} vencen, ${r.resumen.bloqueos} bloqueos, ${r.resumen.cambios} cambios. Edítalo y luego publica.` });
  ...
  async function publicar(tipo: 'apertura' | 'cierre') {
    ...
    try {
      const r = await api<{ url: string; resumen: { hoy: number; vencen: number; bloqueos: number; cambios: number; completadas?: number } }>('/semanas/daily/publicar', { method: 'POST', json: { mesa_id: mesa, tipo, notas: notasArr(), narrativa: narrativa ?? undefined } });
      setMsg({ tipo: 'ok', texto: `${tipo === 'apertura' ? 'Apertura' : 'Cierre'} publicado en Basecamp: ${tipo === 'cierre' ? `${r.resumen.completadas ?? 0} completadas hoy, ${r.resumen.hoy} quedaron abiertas` : `${r.resumen.hoy} para hoy`}, ${r.resumen.vencen} vencen, ${r.resumen.bloqueos} bloqueos, ${r.resumen.cambios} cambios.`, url: r.url });

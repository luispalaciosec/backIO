    ...
    <div className="space-y-4">
      ...
      <header className="no-print flex items-end justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-end gap-2 flex-wrap">
          <div><label className="label">Grupo / preset</label><select className="input w-72" value={grupos.find((g) => g.cliente_ids.join(',') === sel.join(','))?.id ?? ''} onChange={(e) => { const g = grupos.find((x) => x.id === e.target.value); if (g) { setSel(g.cliente_ids); setTitulo(g.nombre); } }}><option value="">Selección manual…</option><optgroup label="Grupos y presets">{grupos.filter((g) => !g.id.startsWith('cliente:')).map((g) => <option key={g.id} value={g.id}>{g.nombre}</option>)}</optgroup><optgroup label="Por cliente">{grupos.filter((g) => g.id.startsWith('cliente:')).map((g) => <option key={g.id} value={g.id}>{g.nombre}</option>)}</optgroup></select></div>
